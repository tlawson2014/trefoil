...
is the v2 design and needs wallet support to be usable. Safe-destination
addresses — limiting where a recovery can send funds, so colluding guardians
gain nothing — are v1.1 and in progress.
...
| `x/recovery/keeper/msg_server_*.go` | The transactions: **set-guardians**, **request-recovery**, **approve-recovery**, **cancel-recovery**. Each file's top comment explains its rules. |
...
**Step 4 — one guardian is not enough.**
